...
async function initChat() {
  ...
  DB.listenUsers(users => {
    // 1. Chỉ lấy người dùng thực có đủ thông tin, không tự chèn Bot
    users = users.filter(usr => usr && usr.name && usr.phone);

    users.forEach(user => {
      // Nếu là người dùng mới (chưa có trong Set) và không phải chính mình
      if (!_knownUserIds.has(user.id)) {
        _knownUserIds.add(user.id); // Đánh dấu đã biết ID này

        if (user.id !== u.id) {
          // Chỉ thông báo "Người dùng mới" nếu đây không phải lần tải đầu tiên (đã có dữ liệu cũ)
  ...
  function renderConvList(filter = '') {
    ...
    if (activeTab === 'chats' && filter) {
      // 1. Lọc cuộc trò chuyện (Bạn bè & Nhóm) tương thích
      ...
      // 2. Lọc người khác (chưa kết bạn) bằng Tên hoặc SDT
    ...
    const friendUsers = allUsers.filter(u => _friendIds.includes(u.id) && (!filter || (u.name && u.name.toLowerCase().includes(filter.toLowerCase()))));
    const allItems = [];
  ...
  function renderFindTab(filter = '') {
    ...
    const others = allUsers.filter(u => !filter || (u.name && u.name.toLowerCase().includes(filter.toLowerCase())) || (u.phone && u.phone.includes(filter)));
    others.forEach(u => {
      let state = 'none';
